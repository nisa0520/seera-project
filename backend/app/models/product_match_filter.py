...
class ProductMatchFilter(Base):
    ...
    weight_mode: Mapped[str] = mapped_column(String(20), nullable=False, default="ROC")
    color_weight_1: Mapped[Optional[float]] = mapped_column(Numeric(8, 6), nullable=True)
    color_weight_2: Mapped[Optional[float]] = mapped_column(Numeric(8, 6), nullable=True)
    color_weight_3: Mapped[Optional[float]] = mapped_column(Numeric(8, 6), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint("weight_mode IN ('ROC','PERCENTAGE')", name="ck_product_match_weight_mode"),
