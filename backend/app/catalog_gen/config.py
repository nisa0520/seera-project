...
# Default colour-count distribution P(k=1..3). The schema caps color_rank at 3
# (rule of three / 60-30-10), so k is bounded to 3 (DOMINANT/SECONDARY/ACCENT).
DEFAULT_COLOR_DISTRIBUTION = (0.50, 0.32, 0.18)
...
class GenConfig:
    ...
    def color_count_weights(self) -> list[float]:
        w = [max(0.0, float(x)) for x in self.color_distribution][:3]
        while len(w) < 3:
            w.append(0.0)
        total = sum(w)
        if total <= 0:
