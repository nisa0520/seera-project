...
GENDER_REVISION = "0002_gender_preferences"
HEAD_REVISION = "head"
...
def _has_image_schema(table_names: set[str]) -> bool:
    return (
        {
        ...
        and _has_column(table_names, "product_visual_assets", "anchor_config")
        and _has_column(table_names, "vton_jobs", "garment_caption")
        and _has_column(table_names, "product_vton_assets", "vton_asset_tier")
    )
...
def ensure_schema_current() -> None:
    ...
    if "alembic_version" not in table_names:
        if table_names and _has_gender_schema(table_names) and _has_image_schema(table_names):
            logger.info("Existing schema already has image feature tables; stamping Alembic head.")
            command.stamp(config, HEAD_REVISION)
            return

        if table_names and _has_gender_schema(table_names):
            logger.info("Existing schema has gender columns but no image tables; stamping gender revision.")
            command.stamp(config, GENDER_REVISION)
        elif table_names:
