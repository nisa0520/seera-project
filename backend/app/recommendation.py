...
def rank_with_saw(items: list[dict], disable_price: bool = False) -> tuple[list[dict], dict]:
    ...
    max_c1 = max(item["skor_produk"] for item in items)
    min_c2 = min(item["price"] for item in items)
    max_c3 = max(item["rating"] for item in items)
    max_c4 = max(item["sold_count"] for item in items)

    for item in items:
        r1 = item["skor_produk"] / max_c1 if max_c1 else 0
        r3 = item["rating"] / max_c3 if max_c3 else 0
        r4 = item["sold_count"] / max_c4 if max_c4 else 0

        if disable_price:
            saw_score = (weights["c1"] * r1) + (weights["c3"] * r3) + (weights["c4"] * r4)
        else:
            r2 = min_c2 / item["price"] if item["price"] else 0
            saw_score = (weights["c1"] * r1) + (weights["c2"] * r2) + (weights["c3"] * r3) + (weights["c4"] * r4)

        item["saw_score"] = round(saw_score, 4)
...
def evaluate_product_colors(y1: float, colors: list[dict]) -> tuple[float, list[float], list[dict]]:
    ordered_colors = sorted(colors, key=lambda c: c["color_order"])
